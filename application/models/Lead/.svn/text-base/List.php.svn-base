<?php
class Application_Model_Lead_List extends Sid_Db_List
{
	...
	private function setSql()
	{
		...
		$sql = ' AND lead_stage < '.Application_Model_Lead::LEAD_STAGE_PROMOTED;

		if(isset($this->params['adm_id']))
		{
			$sql .= " AND lead_adm_id = ".$this->params['adm_id'];
		}
		
		if(isset($this->params['imp_id']))
		{
			$sql .= " AND lead_imp_id = ".$this->params['imp_id'];
		}
		
		
		!isset($this->params['page']) && $this->params['page'] = null;
		!isset($this->params['rows']) && $this->params['rows'] = 9999;
		...
		if($this->params['page'] > 1)
    	{
    		...
    		$this->params['rows'] = $this->params['rows'] > 1 ? $this->params['rows'] : 9999;
	...
	public function find()
	{
		...
		$sql = "SELECT $fields, dsp_name FROM lead_lead
						INNER JOIN dsp_disposition ON (lead_dsp_id = dsp_id)
					WHERE 1 ".$this->setSql();
